...
| What to test | How |
|---|---|
| Hearth & Hird test panel | Host/single-player: `hnh_test enable`, then press `F7`. Configure an NPC or Hearthstone in dropdowns, or use the permanent **Spawn Hearthstone** and **Spawn Start Village** buttons. The panel can disband the Hird or safely despawn its own test NPCs and Hearthstones. See [the multiplayer test guide](docs/MULTIPLAYER_TESTING.md) |
