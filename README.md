...
| What to test | How |
|---|---|
| Hearth & Hird test panel | Host/single-player: `hnh_test enable`, then press `F7`. Configure NPCs or a Hearthstone, or choose any settlement tier from Camp through Jarl's Seat and place it at a surveyed site near you/the first spawn. The panel resets village hostility, disbands the Hird and safely removes its loaded test objects. See [the multiplayer test guide](docs/MULTIPLAYER_TESTING.md) |
